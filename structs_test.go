...
func TestStructs(t *testing.T) {
	...
	if s.EqualContent(0, 0) == diff.True {
		t.Fatalf("Expect to not equal")
	}
	if s.Equal(1, 1) {
		t.Fatalf("Expect to not equal")
	}
	if s.EqualContent(1, 1) == diff.True {
		t.Fatalf("Expect to not equal")
	}
	if !s.Equal(1, 2) {
		t.Fatalf("Expected equal")
	}
	if s.EqualContent(1, 2) != diff.True {
		t.Fatalf("Expected equal")
	}
	if s.Equal(2, 3) {
		t.Fatalf("Expected to not equal (only identity is equal)")
	}
	if s.EqualContent(2, 3) != diff.Identity {
...
func TestStructs2(t *testing.T) {
	...
	if s.Equal(0, 0) {
		t.Fatalf("Expect not to equal (only identity is equal)")
	}
	if s.EqualContent(0, 0) != diff.Identity {
		t.Fatalf("Expect identity")
	}
	if s.Equal(1, 0) {
		t.Fatalf("Expect to not equal")
	}
	if s.EqualContent(1, 0) != diff.False {
		t.Fatalf("Expect to not equal")
	}
	d := diff.NewContent(s)
