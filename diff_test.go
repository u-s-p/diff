...
func TestBackLeft(t *testing.T) {
	d := diff.New(bytesFromString("!!!!!!aaaaaaaaaaaaaaa", "aaaaaaaaaaaaaaa"))
	ts := newTestStruct(t, d, 21)
	ts.expect(diff.Left, 0)
	ts.expect(diff.Left, 1)
	ts.expect(diff.Left, 2)
	ts.expect(diff.Left, 3)
	ts.expect(diff.Left, 4)
	ts.expect(diff.Left, 5)
	ts.expect(diff.Both, 6)
	ts.expect(diff.Both, 7)
	ts.expect(diff.Both, 8)
	ts.expect(diff.Both, 9)
	ts.expect(diff.Both, 10)
	ts.expect(diff.Both, 11)
	ts.expect(diff.Both, 12)
	ts.expect(diff.Both, 13)
	ts.expect(diff.Both, 14)
	ts.expect(diff.Both, 15)
	ts.expect(diff.Both, 16)
	ts.expect(diff.Both, 17)
	ts.expect(diff.Both, 18)
	ts.expect(diff.Both, 19)
	ts.expect(diff.Both, 20)
	ts.end()
}
func TestBackRight(t *testing.T) {
	d := diff.New(bytesFromString("aaaaaaaaaaaaaaa", "!!!!!!aaaaaaaaaaaaaaa"))
	ts := newTestStruct(t, d, 21)
	ts.expect(diff.Right, 0)
	ts.expect(diff.Right, 1)
	ts.expect(diff.Right, 2)
	ts.expect(diff.Right, 3)
	ts.expect(diff.Right, 4)
	ts.expect(diff.Right, 5)
	ts.expect(diff.Both, 0)
	ts.expect(diff.Both, 1)
	ts.expect(diff.Both, 2)
	ts.expect(diff.Both, 3)
